##### DEVELOP STANDARD LINEAR REGRESSION MODELS #####
...
daily = edited[,c("Date","pH","Temp","feed","DO")] %>% filter(Date>=as.POSIXct("2018-01-01"),between(feed,0,7) & 
...
chosen.daily.model = lm(pH~feed1+Temp1+Temp6+pH1+pH2+DO2+DO5,lag_daily); summary(chosen.daily.model)  
...
model = lm(pH~pH1+pH2,train3); summary(model)
